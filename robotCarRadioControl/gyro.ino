...
//  
  ...
  if ((mpuIntStatus & 0x10) || fifoCount == 1024){
    ...
    forwardAcceleration = aaReal.x;
    sideAcceleration = aaReal.y;
    fallAcceleration = aaReal.z;    
    Serial.print(" | forwardAcc  = "); Serial.print(forwardAcceleration);
    Serial.print(" | sideAcc  = "); Serial.print(sideAcceleration);
    Serial.print(" | fallingAcc  = "); Serial.println(fallAcceleration);
    return(forwardAcceleration, sideAcceleration, fallAcceleration);
  }
}
...
float getAngels(){
  ...
  if ((mpuIntStatus & 0x10) || fifoCount == 1024){
    ...
    mpu.dmpGetGravity(&gravity, &q);
    mpu.dmpGetYawPitchRoll(ypr, &q, &gravity);
    rotation = gyroMapping(ypr[0] * 180/M_PI);
    tilt = gyroMapping(ypr[1] * 180/M_PI);
    roll = gyroMapping(ypr[2] * 180/M_PI);
    //Serial.print(" | rotation  = "); Serial.print(rotation);
    //Serial.print(" | tilt  = "); Serial.print(tilt);
    //Serial.print(" | roll  = "); Serial.println(roll);
    //input = ypr[1] * 180/M_PI + 180;
    return(rotation);//, tilt, roll);
  }
}
