...
// Code origin: https://github.com/ownprox/Rotary_Encoder_KY-040_Fixed/tree/main
...
{
  protected:
    const byte DT_Pin, CLK_Pin;
  unsigned int state;
  bool ClickDown;
  const CallbackFunc RotaryCallback;

  public:
    RotaryEncoder(const CallbackFunc RotaryCallback, const byte DT_Pin = 2, const byte CLK_Pin = 3) : RotaryCallback(RotaryCallback), DT_Pin(DT_Pin), CLK_Pin(CLK_Pin)
    {
